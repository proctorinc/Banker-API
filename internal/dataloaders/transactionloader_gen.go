...
type TransactionLoaderConfig struct {
	...
	Fetch func(keys []string) ([][]db.Transaction, []error)
...
type TransactionLoader struct {
	...
	cache map[string][]db.Transaction
...
func (l *TransactionLoader) Load(key string) ([]db.Transaction, error) {
...
func (l *TransactionLoader) LoadThunk(key string) func() ([]db.Transaction, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]db.Transaction, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *TransactionLoader) LoadAll(keys []string) ([][]db.Transaction, []error) {
	results := make([]func() ([]db.Transaction, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	transactions := make([][]db.Transaction, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TransactionLoader) LoadAllThunk(keys []string) func() ([][]db.Transaction, []error) {
	results := make([]func() ([]db.Transaction, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TransactionLoader) Prime(key string, value []db.Transaction) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]db.Transaction, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *TransactionLoader) unsafeSet(key string, value []db.Transaction) {
	if l.cache == nil {
		l.cache = map[string][]db.Transaction{}
	}
	l.cache[key] = value
}
